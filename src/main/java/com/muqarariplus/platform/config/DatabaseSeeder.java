...
public class DatabaseSeeder implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        seedUsers();
        seedExperts();
        seedAcademicHierarchy(); // من كود الزملاء (الهيكل الأكاديمي)
        seedCourses();           // من كود الزملاء (المقررات المرتبطة بالتخصصات)
        seedSiteContent();       // من كودك (ترجمات السوبر آدمن الشاملة)
        seedIndustryHierarchy(); // من كودك (المهارات والأدوات الضخمة)
        seedCertifications();    // من كودك (الشهادات العالمية الضخمة)
        purgeNonTechnicalEnrichments(); // من كودك (بروتوكول التطهير)
        List<User> massiveExperts = seedMassiveExperts(); // من كودك (الخبراء الحقيقيون)
        seedMassiveEnrichments(massiveExperts);           // من كودك (المحرك الأعظم لتوليد الإثراءات)
    }
    ...
    // ── الهيكل الأكاديمي (من كود الزملاء) ──
    private void seedAcademicHierarchy() {
        ...
        System.out.println("SEEDER: Academic hierarchy seeded.");
    }

    // ── المقررات الأكاديمية وربطها بالتخصصات (من كود الزملاء) ──
    private void seedCourses() {
        ...
        for (Major major : allMajors) {
            for (String[] c : universityReqs) {
                ...
                co.setCode(c[0]); co.setNameEn(c[1]); co.setNameAr(c[2]);
        ...
        System.out.println("SEEDER: Courses seeded with major relationships.");
    }

    // ── نصوص الموقع والترجمات الضخمة (من كودك الشامل) ──
    ...
    private void seedIndustryHierarchy() {
        ...
        System.out.println("SEEDER: " + newSkills + " new Skills added.");
        ...
        System.out.println("SEEDER: " + newTools + " new Tools added.");
    }
    ...
    private void seedCertifications() {
        ...
        System.out.println("SEEDER: " + newCerts + " new Certifications added.");
    }

    // ── دوال التطهير ومحرك الإثراء الأعظم (من كودك) ──
    ...
    private void purgeNonTechnicalEnrichments() {
        ...
        for (Course course : allCourses) {
            ...
            for (CourseEnrichment e : approved) {
                ...
                if (contaminated) toDelete.add(e);
            }
            if (!toDelete.isEmpty()) {
        ...
        entityManager.flush();
        entityManager.clear();
        System.out.println("SEEDER [PURGE]: Removed " + purgedNonTech + " non-tech + " + purgedStale + " stale cross-domain.");
    }
    ...
    private List<User> seedMassiveExperts() {
        ...
        System.out.println("SEEDER: " + expertData.length + " Experts seeded.");
    ...
    private void seedMassiveEnrichments(List<User> experts) {
        ...
        System.out.println("SEEDER: " + totalNew + " enrichments dynamically generated.");
    }
